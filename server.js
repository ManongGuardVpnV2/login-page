// server.js — obfuscated injected script, credentials included, no external deps
...
// sample channels if you don't have channels.json; optional
const CHANNELS = [
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === "/validate-token" && method === "POST") {
    ...
    req.on("end", () => {
      try {
        ...
        // no Secure flag so local http testing works. Add Secure in production.
  ...
  // channels - protected endpoint (if you have public/channels.json prefer that)
  if (url.pathname === "/channels" && method === "GET") {
    const sid = getCookie(req, "sessionId");
    if (!validateSession(sid)) return sendJSON(res, 401, { success: false, error: "Unauthorized" });
    // if public/channels.json exists, serve it; otherwise return CHANNELS above
  ...
  if (url.pathname === "/iptv" && method === "GET") {
    ...
    // obfuscated compact script: all fetches use credentials:'include'
    // array-based keys to make it harder to read in DevTools
    const obf = `
(function(){
  try{
    ...
    async function r(){ try{ await fetch('/'+_[9],{method:_[10], [_[18]]:_[19]}); }catch(e){} }
    v().then(function(j){ if(!j[_[4]]){ location.href='/'; return; } E=j[_[5]]; s(); setInterval(r,5*60*1000);
      // fetch protected channels and hand to page's handler
      fetch('/'+_[7],{cache:'no-store', [_[18]]:_[19]}).then(function(r2){ return r2[_[2]](); }).then(function(ch){
...
(async function(){
  ...
  // serve static files under public (css/js/json)
